...
function assertAssistantMountPreservesRoutes(mainSource) {
  ...
  const browserContents = mainSource.match(/<BrowserRouter>([\s\S]*?)<\/BrowserRouter>/)?.[1];
  assert.notEqual(browserContents, undefined);
  assert.match(browserContents.split('<Routes>')[0], /<ChatWidget\s*\/>/);
