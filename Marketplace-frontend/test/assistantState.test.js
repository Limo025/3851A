...
test('starts with a closed, empty assistant conversation', () => {
  assert.deepEqual(initialChatState(), {
    ...
    loading: false,
    error: '',
    responseDetails: { inventorySummary: undefined, listings: [], draftReady: false },
  });
});
...
test('a new request clears stale listing results and draft review action', () => {
  const ready = assistantReducer(initialChatState(), {
    type: 'assistant-received',
    payload: {
      message: 'Your draft is ready.',
      state: { mode: 'sell', stage: 'draft_ready', criteria: {}, draft: { title: 'Desk' } },
      listings: [{ id: 'old-listing' }],
      inventorySummary: { total: 1 },
      draftReady: true,
    },
  });

  assert.equal(ready.responseDetails.draftReady, true);
  const pending = assistantReducer(ready, {
    type: 'assistant-submitted',
    payload: { message: 'Show me a PS5 instead' },
  });
  assert.deepEqual(pending.responseDetails, {
    inventorySummary: undefined,
    listings: [],
    draftReady: false,
  });

  const failed = assistantReducer(pending, {
    type: 'assistant-failed',
    payload: { error: 'Assistant unavailable' },
  });
  assert.equal(failed.responseDetails.draftReady, false);
  assert.equal(failed.error, 'Assistant unavailable');
});
