...
import { clearListingDrafts, consumeListingDraft, storeListingDraft } from '../src/assistant/draftHandoff.js';
...
test('two render reads keep the draft until the page commits', () => {
  const assistantDraftId = storeListingDraft({
    title: 'PS5 console',
    description: 'PS5 console in good working condition.',
    price: 450,
  ...
  const state = { assistantDraftId };

  const firstRender = listingDraftFromLocationState(state);
  const secondRender = listingDraftFromLocationState(state);

  assert.deepEqual(secondRender, firstRender);
  assert.equal(firstRender.title, 'PS5 console');
  consumeListingDraft(assistantDraftId);
  assert.equal(listingDraftFromLocationState(state), undefined);
});
